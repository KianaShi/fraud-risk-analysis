...
class ModelingTests(unittest.TestCase):
    ...
    def test_stratified_split_is_reproducible_and_balanced(self) -> None:
        ...
        first = stratified_split(target, random_state=42)
        second = stratified_split(target, random_state=42)
    ...
    def test_split_does_not_depend_on_open_date_order(self) -> None:
        ...
        partitions = stratified_split(target, random_state=42)
    ...
    def test_split_audit_reports_counts_and_prevalence(self) -> None:
        ...
        target = pd.Series([0, 1] * 50)
        audit = split_audit(target, stratified_split(target))
    ...
    def test_validation_and_test_rows_are_never_passed_to_fit(self) -> None:
        ...
        raw = pd.DataFrame(
            {
                "is_fraud": [0, 1] * (size // 2),
                "ea_score": np.arange(size, dtype=float),
            }
        )
    ...
    def test_full_benchmark_reports_both_models_and_prevalence(self) -> None:
        ...
        raw = pd.DataFrame(
            {
                "open_date": pd.date_range("2024-01-01", periods=size, freq="D"),
                "is_fraud": np.arange(size) % 2,
                "ea_score": np.linspace(0, 10, size),
                "is_valid": np.arange(size) % 2,
                "type": np.where(np.arange(size) % 3 == 0, "A", "B"),
            }
        )
